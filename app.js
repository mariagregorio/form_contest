...
// TODO add multiple trunk property, save array of trunk ids and channels values
...
function trunkOptionToggle(event) {
	...
	if(event.target.selected == true) {
		...
		if (initial_data.multi_trunk.length > 0) {
			...
			// TODO remove channel slider for removed trunk 
		}
	} else {
		...
		// TODO assign channel slider for added trunk 
		let multi_channels_cont = document.getElementById('multi_channels');

		let new_channels_slider = '<div id="channels_cont'+ parseInt(event.target.value) +'" class="form-group">' + 
		'<label for="formControlRange" class="channels-label">Trunk '+ event.target.value + ' | Channels: '+
		'<i onclick="updateMultiChannelsValue(\'minus\',' + parseInt(event.target.value) +')" id="channelsMinus" class="fas fa-minus"></i>'+
		'<span id="channelsDisplay'+ parseInt(event.target.value) +'">1</span>'+
		...
		'onchange="setChannelsMulti(event, '+ parseInt(event.target.value) +')"></div>'
		multi_channels_cont.innerHTML = multi_channels_cont.innerHTML + new_channels_slider;
	}
}
...
function setChannelsMulti(event, trunkid) {
	...
	document.getElementById('channelsDisplay' + trunkid).innerText = trunkid;
	document.getElementById('channelsDisplay'+ trunkid).innerText = event.target.value;
}

function updateChannelMulti(trunkid) {
	
}



// TODO PLUS MINUS FUNCTIONALITYYYY"!!""


function updateMultiChannelsValue(op, trunkid) {
	
	let channelsRangeInput = document.getElementById('channels' + trunkid);
console.log(channelsRangeInput.value);
	if (op == 'plus') {
		console.log('plusss')
		channelsRangeInput.value = parseInt(channelsRangeInput.value) + 1;
	} else if (op == 'minus') {
	...
	document.getElementById('channelsDisplay' + trunkid).innerText = channelsRangeInput.value;
}